...
if DispFlag
    ...
    for i = 1:length(uniqueAges)
        ...
        if ~isempty(dataGroup)
            ...
            rectangle('Position', [boxPositions(i) - boxWidths/2, q1, boxWidths, iqr], 'FaceColor', [0.7 0.7 0.7], 'EdgeColor', 'k', 'LineWidth', 1.5);
            line([boxPositions(i) - boxWidths/2, boxPositions(i) + boxWidths/2], [med, med], 'Color', 'r', 'LineWidth', 2);
            ...
            line([boxPositions(i) - boxWidths/4, boxPositions(i) + boxWidths/4], [lowerWhisker, lowerWhisker], 'Color', 'k', 'LineWidth', 1);
            line([boxPositions(i) - boxWidths/4, boxPositions(i) + boxWidths/4], [upperWhisker, upperWhisker], 'Color', 'k', 'LineWidth', 1);
...
% 添加强力保护措施，解决Excel打印为空白（NaN）的问题
d_years = data.DeathYear(:);
d_gends = data.Gender(:);

if (n - female) > 0
    male_years = d_years(d_gends == 1);
    if sum(isnan(male_years)) < length(male_years)
        SummaryVariable{3,1} = round(sum(male_years, 'omitnan')/(n-female)*100)/100 -1; 
    else
...
if female > 0
    female_years = d_years(d_gends == 2);
    if sum(isnan(female_years)) < length(female_years)
        SummaryVariable{4,1} = round(sum(female_years, 'omitnan')/female*100)/100 -1;
    else
...
String{2}=sprintf('age: all: %g, male: %g, female: %g', round(sum(data.DeathYear)/n*100)/100 -1,... % year adapted
    SummaryVariable{3,1},...                                            
    SummaryVariable{4,1});                                                  % year adapted
    ...
    if (BM_year(f) >5) && (BM_year(f) <95)
        ...
        elseif isequal(Flag, 'Cancer')
            if BM_year(f) >20  % we ignore benchmarks for age 1-20
                ...
                if and(BM.value{bmc} >= BM.benchmark{bmc}*(1 - tolerance), BM.value{bmc} <= (BM.benchmark{bmc}*(1 + tolerance)))
                    BM.flag{bmc} = 'green';
                elseif abs(BM.value{bmc} - BM.benchmark{bmc}) <= max(2, 0.05*BM.benchmark{bmc}) % allow larger absolute tolerance for high incidence
                    BM.flag{bmc} = 'green';
                else
                    BM.flag{bmc} = 'red';
                end
