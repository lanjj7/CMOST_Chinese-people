...
if isempty(b)
    % 如果找不到标题，尝试查找所有 axes 并匹配标题
    all_axes = findall(a, 'type', 'axes');
    for i = 1:length(all_axes)
        ax_title = get(get(all_axes(i), 'Title'), 'String');
        if strcmp(ax_title, GraphTitle)
            b = get(all_axes(i), 'Title');
            break;
        end
    end
end
...
if ~isempty(b) && all(isvalid(b))
    if isprop(b, 'Parent') && all(isvalid(b.Parent))
        ax_handle = b.Parent;
    else
        ax_handle = ancestor(b, 'axes');
    end
else
    % 如果找不到，尝试直接查找坐标区
    ax_handle = findall(a, 'type', 'axes', '-depth', 1);
    if ~isempty(ax_handle)
        ax_handle = ax_handle(1);
    end
end

% 确保 ax_handle 是有效的坐标区
if ~isempty(ax_handle) && all(isvalid(ax_handle)) && strcmp(get(ax_handle, 'type'), 'axes')
    axes(ax_handle);
    cla(ax_handle);
    hold on;
    plot(0:99, DataGraph, 'color', 'k')
...
if ~isempty(ax_handle) && all(isvalid(ax_handle)) && strcmp(get(ax_handle, 'type'), 'axes')
    ...
    cla(ax_handle);
    hold on;
    plot(Population,   '--ks','LineWidth',LineSz, 'MarkerEdgeColor','k', 'MarkerFaceColor','m', 'MarkerSize',MarkerSz)
...
% 辅助函数：查找并激活坐标区
function ax_handle = FindAndActivateAxes(b, ~)
    ax_handle = [];
    ...
    % 激活坐标区
    if ~isempty(ax_handle) && all(isvalid(ax_handle))
        try
            axes(ax_handle);
            cla(ax_handle);
        catch
