...
%
%     This program is part of free so.ftware package CMOST for colo-rectal  
%     cancer simulations: You can redistribute it and/or modify 
%     it under the terms of the GNU General Public License as published by
%     the Free Software Foundation, either version 3 of the License, or
%     (at your option) any later versihanon.
% 
%       
...
% 加载温州ADR数据的基准参数（从Default_Benchmarks.m）
handles = Default_Benchmarks(handles);

handles.Variables.NumberPatientsValues = [1000 5000 10000 25000 50000 100000 200000 500000 1000000 2000000 5000000];
set(handles.Number_patients, 'String', cellstr(num2str(handles.Variables.NumberPatientsValues(:))));
...
if ~isfield(handles, 'Number_patients') || ~isvalid(handles.Number_patients)
    return;
end
...
try
    ...
    % GUI objects may have been deleted
    return;
end
...
% handles.Variables.DispFlag = true; % 已修改: 不再强制覆盖 PDF 设置，而是遵从 GUI 勾选状态
...
% 保存当前GUI的输出设置，用于多任务批量运行
CurrentExcelFlag   = handles.Variables.ExcelFlag;
CurrentResultsFlag = handles.Variables.ResultsFlag;
CurrentDispFlag    = handles.Variables.DispFlag;

% Prepare variables for parfor
Starter = handles.Variables.Starter;
NumIterations = length(Starter.CurrentSummary);

try
    for f = 1 : NumIterations
        % Load variables inside the loop
        % We cannot access handles directly
        
        % Construct full path
        fullPath = fullfile(Starter.CurrentPath{f}, Starter.CurrentSummary{f});
        
        % Load data
        LoadedData = importdata(fullPath);
        
        % Check if it's a struct and has Variables (based on Load_Callback logic)
        % But here we assume it returns the Variables struct directly as per original code:
        % handles.Variables = importdata(...)
        
        CurrentVariables = LoadedData;
        
        % Update Variables
        CurrentVariables.StarterFlag = 'on';
        CurrentVariables.Starter = Starter;
        CurrentVariables.Starter.Counter = f;
        CurrentVariables.ResultsPath = ResultsPath;
        
        % 使用当前GUI的输出设置，而非加载文件中的设置
        % 这样才能确保Excel汇总输出正常工作
        CurrentVariables.ExcelFlag   = CurrentExcelFlag;
        CurrentVariables.ResultsFlag = CurrentResultsFlag;
        CurrentVariables.DispFlag    = CurrentDispFlag;
        
        % Create a dummy handles structure
        % CalculateSub expects handles.Variables
        dummyHandles = struct();
        dummyHandles.Variables = CurrentVariables;
        
        % Run calculation
        % We ignore the returned handles as they are not used in the original loop
        % except for the next iteration which overwrites them.
        % The side effect is saving results to disk in Evaluation_PBP_IndCosts
        CalculateSub(dummyHandles); 
    end
