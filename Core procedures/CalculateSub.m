...
for f=1:length(ScreeningHandles)
    if handles.Variables.Screening.(ScreeningHandles{f})(1) > 0
        Ende = Start + round(handles.Variables.Screening.(ScreeningHandles{f})(1) * 1000) - 1;
...
if useGPU
    ...
    r1 = gpuArray.rand(n, 1);
    r2 = gpuArray.rand(n, 1);
    r3 = gpuArray.rand(n, 1);
    
    % Calculate IndividualRisk
    IndividualRisk = handles.Variables.IndividualRisk(round(r1*499)+1);
    
    % Calculate Gender
    Gender = ones(n, 1);
    Gender(r2 < handles.Variables.fraction_female) = 2;
    
    % Calculate ScreeningPreference
    ScreeningPreference = ScreeningMatrix(round(r3*999)+1);
    
    % Transfer back to CPU if needed for later processing
    IndividualRisk = gather(IndividualRisk);
    Gender = gather(Gender);
    ScreeningPreference = gather(ScreeningPreference);
else
    for f=1:n
...
clear p StageVariables Location Cost CostStage risc flag SpecialText female Sensitivity
clear ScreeningTest ScreeningPreference AgeProgression NewPolyp ColonoscopyLikelyhood
clear IndividualRisk RiskDistribution Gender LifeTable LocationMatrix StageDuration
clear tx1 DirectCancerRate DirectCancerSpeed DwellSpeed PBP stats All
