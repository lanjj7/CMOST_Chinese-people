...
handles.Variables.Number_patients = 50000;  % 增加模拟人数减少噪声
handles = Step1_Subfunction(handles, 30, 1200); % 增加迭代次数改善收敛

handles.Variables.Number_patients = 80000;
handles = Step2_Subfunction(handles, 40, 1000); % 增加迭代次数

handles.Variables.Number_patients = 150000;
% Step3 (carcinoma) 仅在显式启用时运行，避免默认覆盖 Step2
if isfield(handles.Variables, 'RunStep3') && isequal(handles.Variables.RunStep3, true)
    handles = Step3_Subfunction(handles, 40, 1000); % 增加迭代次数
else
    disp('Skipping Step3 (carcinoma calibration). To enable set handles.Variables.RunStep3 = true and rerun.');
end

handles.Variables.Number_patients = 150000;
% Step23（联合优化）仅在显式启用时运行，默认不执行以避免覆盖第2步结果
if isfield(handles.Variables, 'RunStep23') && isequal(handles.Variables.RunStep23, true)
    handles = Step23_Subfunction(handles, 150); % 增加联合优化次数
else
    disp('Skipping Step23 (joint optimization). To enable set handles.Variables.RunStep23 = true and rerun.');
end
...
% 同步百分位数值，以便 GUI 能够正确显示校准后的结果
Indices = [50 100 150 200 250 300 350 400 450 475 485 500];
for k = 1:length(Indices)
    handles.Variables.Ind_Risk_Percentiles(k) = handles.Variables.IndividualRisk(Indices(k));
end

% 使用五参数广义Logistic函数（Richards曲线），改善首尾拟合
% A(1)=上界, A(2)=斜率, A(3)=中点, A(4)=下界, A(5)=不对称参数
richards5 = @(A,u) A(4) + (A(1)-A(4))./((1 + exp(-A(2)*(u-A(3)))).^A(5));
% 带权重的拟合：首尾数据点权重更高
fit_richards5 = @(u,y,p0) fminsearch(@(p) weighted_sse_richards(p, u, y, richards5), p0, ...
    optimset('TolX',1e-10,'TolFun',1e-10,'MaxIter',8000,'MaxFunEvals',20000));
% 初始参数：[上界, 斜率, 中点, 下界, 不对称参数]
y_data = handles.Variables.Benchmarks.EarlyPolyp.Ov_perc;
x_data = handles.Variables.Benchmarks.EarlyPolyp.Ov_y;
% 智能初始化参数
p0_5param = [max(y_data)*1.05, 0.08, median(x_data), min(y_data)*0.3, 1.0];
B5 = fit_richards5(x_data, y_data, p0_5param);
% 确保参数合理
if B5(4) < 0, B5(4) = 0; end
if B5(5) < 0.1, B5(5) = 0.1; end
if B5(5) > 5, B5(5) = 5; end
% 转换为3参数格式以保持兼容性
B = [B5(1)-B5(4), B5(2), B5(3)];
CoeffsEarly{1} = B;
EEarly{20} = B;
% 保存完整的Richards参数供后续使用（存放在 Step1 子字段）
if ~isfield(handles.Variables, 'Calibration') || ~isstruct(handles.Variables.Calibration)
    handles.Variables.Calibration = struct();
end
handles.Variables.Calibration.Step1.SigmoidLowerBound = B5(4);
handles.Variables.Calibration.Step1.RichardsAsymmetry = B5(5);
handles.Variables.Calibration.Step1.RichardsParams = B5;
...
B = fit_sigmoid(handles.Variables.Benchmarks.EarlyPolyp.Ov_y, BMEarlyy{i}, [50 1 50]);
CoeffsEarly{i} = B;
EEarly{i}      = B;
...
for i=2:NumberFirstIteration
    ...
    % we adjust early polyp parameters（改用 fminsearch 拟合）
    B = fit_sigmoid(handles.Variables.Benchmarks.EarlyPolyp.Ov_y, BMEarlyy{i-1}, [50 1 50]);
    
    % 计算首尾误差，用于自适应学习率
    headError = abs(BMEarlyy{i-1}(1) - handles.Variables.Benchmarks.EarlyPolyp.Ov_perc(1)) / ...
        max(handles.Variables.Benchmarks.EarlyPolyp.Ov_perc(1), 0.1);
    tailError = abs(BMEarlyy{i-1}(end) - handles.Variables.Benchmarks.EarlyPolyp.Ov_perc(end)) / ...
        max(handles.Variables.Benchmarks.EarlyPolyp.Ov_perc(end), 0.1);
    
    % 自适应学习率：误差大时学习率更大
    adaptiveLR1 = 0.04 * (1 + 0.5*max(headError, tailError));
    adaptiveLR2 = 0.8 * (1 + 0.3*max(headError, tailError));
    adaptiveLR3 = 0.5 * (1 + 0.3*max(headError, tailError));
    
    CoeffsEarly{i}(1) = CoeffsEarly{i-1}(1)*exp(-adaptiveLR1*(EEarly{i-1}(1)-EEarly{20}(1))) * exp(-adaptiveLR1*BM_OvPop(1)/BMPop{i-1}(1)) ;
    CoeffsEarly{i}(2) = CoeffsEarly{i-1}(2)-adaptiveLR2*(EEarly{i-1}(2)-EEarly{20}(2)) ;
    CoeffsEarly{i}(3) = CoeffsEarly{i-1}(3)-adaptiveLR3*(EEarly{i-1}(3)-EEarly{20}(3)) ;
    
    % 更新下界参数以改善首部拟合（使用 Step1 子字段）
    if isfield(handles.Variables, 'Calibration') && isfield(handles.Variables.Calibration, 'Step1') && isfield(handles.Variables.Calibration.Step1, 'SigmoidLowerBound')
        firstBenchmark = handles.Variables.Benchmarks.EarlyPolyp.Ov_perc(1);
        if headError > 0.15  % 首部误差大于15%时调整下界
            currentLB = handles.Variables.Calibration.Step1.SigmoidLowerBound;
            if BMEarlyy{i-1}(1) > firstBenchmark
                % 模拟值偏高，减小下界
                handles.Variables.Calibration.Step1.SigmoidLowerBound = currentLB * 0.85;
            else
                % 模拟值偏低，增大下界
                handles.Variables.Calibration.Step1.SigmoidLowerBound = currentLB * 1.1;
            end
            % 确保下界不超过第一个基准点的60%
            handles.Variables.Calibration.Step1.SigmoidLowerBound = min(...
                handles.Variables.Calibration.Step1.SigmoidLowerBound, 0.6 * firstBenchmark);
            % 确保下界非负
            handles.Variables.Calibration.Step1.SigmoidLowerBound = max(...
                handles.Variables.Calibration.Step1.SigmoidLowerBound, 0);
        end
    end
...
B=my_nlinfit(handles.Variables.Benchmarks.EarlyPolyp.Ov_y,BMEarlyy{i},@(A, u)(A(1)./(1 + exp(-(u*A(2)-A(3))))),[50 1 50]);
    EEarly{i} = B;
end
...
options = optimset('MaxFunEvals', NumberSecondIteration, 'MaxIter', NumberSecondIteration, ...
...
handles.Variables.Calibration.Step1.RMSE    = RMSE(end);
handles.Variables.Calibration.Step1.RMSP1   = RMSP(end);
handles.Variables.Calibration.Step1.RMSMPop = RMSMPop(end);
handles.Variables.Calibration.Step1.CoeffEar  = CoeffsEarlyFinal;
...
CoeffsAdv{1} = my_nlinfit(Benchmark_AdvAd_y,Benchmark_AdvAd_perc,@(A, u)(A(1)./(1 + exp(-(u*A(2)-A(3))  ))),[6 0.5 7]);
...
if AdFlag
    ...
    handles.Variables.EarlyProgressionRate = 0.0064*CoeffsAdv{1}(1).*exp(-0.01*CoeffsAdv{1}(2)*( index_age - CoeffsAdv{1}(3) ).^2); %mmmm 4*
    counter = 1;
    for x1=1:19
        for x2=1:5
...
% CoeffsAdv{i+1}(1) = CoeffsAdv{i}(1)*exp(-0.001*(EAdv{i}(1)-EAdv_Start(1)));
CoeffsAdv{i+1}(1) = CoeffsAdv{i}(1)*exp(-0.0005*(EAdv{i}(1)-EAdv_Start(1)));
...
for i=2:NumberFirstIteration
    ...
    % CoeffsAdv{i+1}(3) = CoeffsAdv{i}(3)+0.1*(EAdv{i}(3)-EAdv_Start(3));
    CoeffsAdv{i+1}(1) = CoeffsAdv{i}(1)*exp(-0.19*(EAdv{i}(1)-EAdv_Start(1))); %m -0.06*  %m 0.19
    CoeffsAdv{i+1}(2) = CoeffsAdv{i}(2)-0.01*(EAdv{i}(2)-EAdv_Start(2));
    CoeffsAdv{i+1}(3) = CoeffsAdv{i}(3)+0.01*(EAdv{i}(3)-EAdv_Start(3));
...
handles.Variables.Calibration.Step2.RMSA    = RMSA;
handles.Variables.Calibration.Step2.RMSP1   = RMSP; 
handles.Variables.Calibration.Step2.CoeffAdv  = CoeffsAdv_Final;
...
% Backup Step2 results so user can restore or re-run Step2 independently
try
    Path = mfilename('fullpath'); pos = regexp(Path, [mfilename, '$']); CurrentPath = Path(1:pos-1);
    BackupPath = fullfile(CurrentPath, '..', 'Storyboards', 'Temp');
    if ~exist(BackupPath, 'dir'), mkdir(BackupPath); end
    save(fullfile(BackupPath, 'Calibration_2_result.mat'), 'CoeffsAdv_Final', 'ProgressionCoefficient', 'FemFactorAdv', 'RMSA', 'RMSP');
    disp(['Saved Step2 backup to ' fullfile(BackupPath, 'Calibration_2_result.mat')]);
catch
    save('Calibration_2_result.mat', 'CoeffsAdv_Final', 'ProgressionCoefficient', 'FemFactorAdv', 'RMSA', 'RMSP');
    disp('Saved Step2 backup to Calibration_2_result.mat in current directory');
end
...
% Backup variables to prevent Step 3 from overwriting Step 1/2 calibration results
VarsBackup = handles.Variables;
...
while min(RMSI) >5 % here we keep trying until a good initial guess is reached
    ...
    CoeffsInc{i} = my_nlinfit(Benchmark_Ca_y,Benchmark_Ca_inc,@(A, u)(A(1)./(1 + exp(-(u*A(2)-A(3))  ))),[10 1 10]);
    ...
    lastwarn('')
    B=my_nlinfit(BMIncx{i},BMIncy{i},@(A, u)(A(1)./(1 + exp(-(u*A(2)-A(3))   ))),[10 1 10]);
    if ~isempty(lastwarn) || ~all(isfinite(B))
        B=my_nlinfit(BMIncx{i},BMIncy{i},@(A, u)(A(1)./(1 + exp(-(u*A(2)-A(3))   ))),[10 -1 -10]);
    end
    ...
    for i=2:NumberFirstIteration
        ...
        CoeffsInc{i+1}(1) = CoeffsInc{i}(1)*exp(-0.00001*EAdv_Start(1)/EAdv{i}(1)*(EInc{i}(1)-EInc_Start(1))); % coupled  %0.002*2
        CoeffsInc{i+1}(2) = CoeffsInc{i}(2)-0.1*(EInc{i}(2)-EInc_Start(2));
        CoeffsInc{i+1}(3) = CoeffsInc{i}(3)-0.5*(EInc{i}(3)-EInc_Start(3));
...
% Run final adjustments to compute BM and RMS, but keep a backup of Step1/2 variables
clear tmp2
tmp2{1} = CoeffsInc_Final;

if CaFlag
    % Apply rate adjustments to produce final BM (temporary modification)
...
% run the calculations again to get final BM and RMS
[~, BM] = CalculateSub(handles);

% compute RMS for step3
[RMSI, RMSD, RMSR, BMAdvx, BMAdvy, BMIncx, BMIncy] = CalculateRMS_Step3(handles, BM,...
    BMAdvx, BMAdvy, BMIncx, BMIncy, Benchmark_Ca_inc, RMSI, RMSD, RMSR, i, CaFlag, RelDangerFlag, AdjFractionRectumFlag); %#ok<ASGLU>

% Save results into a dedicated structure to avoid overwriting Step1/2 variables
step3_results.CoeffsInc = CoeffsInc_Final;
step3_results.FemFactorCa = femFinal;
step3_results.CoeffRelDanger = fastCancerFinal;
step3_results.Rectum = rectumFinal;
step3_results.RMSI = RMSI(end);
step3_results.RMSD = RMSD(end);
step3_results.RMSR = RMSR(end);

% Restore original variables from backup so Step1/2 results remain unchanged
handles.Variables = VarsBackup;

% Store Step3 results in the Calibration namespace (dedicated fields)
handles.Variables.Calibration.Step3.CoeffsInc = step3_results.CoeffsInc;
handles.Variables.Calibration.Step3.CoeffsInc_FemFactor = step3_results.FemFactorCa;
handles.Variables.Calibration.Step3.CoeffsInc_FastCancer = step3_results.CoeffRelDanger;
handles.Variables.Calibration.Step3.CoeffsInc_Rectum = step3_results.Rectum;
handles.Variables.Calibration.Step3.RMSI = step3_results.RMSI;
handles.Variables.Calibration.Step3.RMSD = step3_results.RMSD;
handles.Variables.Calibration.Step3.RMSR = step3_results.RMSR;

display(sprintf('Calibration Step 3 finished'))  
display(sprintf('RMS carcinoma incidence %.4f', step3_results.RMSI))
display(sprintf('RMS danger adenoma %.4f', step3_results.RMSD))
display(sprintf('RMS rectum cancer %.4f', step3_results.RMSR))
...
% 获取下界参数和不对称参数（如果存在，来自 Step1 子字段）
if isfield(handles.Variables, 'Calibration') && isfield(handles.Variables.Calibration, 'Step1') && isfield(handles.Variables.Calibration.Step1, 'SigmoidLowerBound')
    lowerBound = handles.Variables.Calibration.Step1.SigmoidLowerBound;
else
    lowerBound = 0;
end

% 获取Richards曲线的不对称参数
if isfield(handles.Variables, 'Calibration') && isfield(handles.Variables.Calibration, 'Step1') && isfield(handles.Variables.Calibration.Step1, 'RichardsAsymmetry')
    asymmetry = handles.Variables.Calibration.Step1.RichardsAsymmetry;
else
    asymmetry = 1.0;  % 默认对称sigmoid
end

% 确保下界参数不会导致年轻人群患病率过高
firstBenchmark = handles.Variables.Benchmarks.EarlyPolyp.Ov_perc(1);
if lowerBound > 0.6 * firstBenchmark
    lowerBound = 0.6 * firstBenchmark;
    handles.Variables.Calibration.Step1.SigmoidLowerBound = lowerBound;
end

if isequal(i, 1)
    % 使用改进的 Richards 公式，考虑下界偏移和不对称性
    baseRate = 0.1*0.90*0.07/50*exp(-0.0*(handles.Variables.Benchmarks.EarlyPolyp.Ov_perc(end)));
    sigmoidTerm = 1./(1 + exp(-((index_age*5*CoeffsEarly(2))-14*CoeffsEarly(2)-CoeffsEarly(3)))).^asymmetry;
    handles.Variables.NewPolypRate = baseRate * (lowerBound + CoeffsEarly(1) .* sigmoidTerm);
else
    baseRate = 0.3*0.90*0.07/50*exp(-0.0*(handles.Variables.Benchmarks.EarlyPolyp.Ov_perc(end)));
    sigmoidTerm = 1./(1 + exp(-((index_age*5*CoeffsEarly(2))-14*CoeffsEarly(2)-CoeffsEarly(3)))).^asymmetry;
    handles.Variables.NewPolypRate = baseRate * (lowerBound + CoeffsEarly(1) .* sigmoidTerm);
end
counter = 1;
...
if CaFlag
    ...
    % 改进模型：Logistic + 高年龄衰减项，与Auto_Calib_3_TempFunction同步
    
    age_values = index_age * 5;  % 转换为实际年龄
    base_scaling = 2.6e-4;
    ...
    age_decay_start = 70;
    decay_rate = 0.0019;
    extra_decay_start = 82;
    extra_decay_rate = 0.0035;
    ...
    handles.Variables.AdvancedProgressionRate = base_scaling * CoeffsInc{i}(1) * logistic_term .* decay_factor .* extra_decay_factor;
...
RMSE(i) = 0;
n_points = length(handles.Variables.Benchmarks.EarlyPolyp.Ov_y);
for j=1:n_points
    % 分段权重：首部(1-3)、尾部(n-2:n)权重更高，改善边缘拟合
    if j <= 2
        weight = 3.5;  % 最前两个点权重最高
    elseif j == 3
        weight = 2.5;  % 第三个点中等高权重
    elseif j >= n_points - 1
        weight = 3.0;  % 最后两个点高权重
    elseif j == n_points - 2
        weight = 2.0;  % 倒数第三个点中等权重
    else
        weight = 1.0;  % 中间点正常权重
    end
    RMSE(i) = RMSE(i) + weight*(1 - BMEarlyy{i}(j)/handles.Variables.Benchmarks.EarlyPolyp.Ov_perc(j))^2;
end
...
% 改进模型：Logistic + 高年龄衰减项
age_values = index_age * 5;
base_scaling = 2.6e-4;
...
% we calculate RMS for carcinoma incidence - 改进版本，增加高年龄段权重
RMSI=0;
ageYears = handles.Variables.Benchmarks.Cancer.Ov_y;
for j=5:length(ageYears)
    if ~isequal(handles.Variables.Benchmarks.Cancer.Ov_inc(j),0)
        % 增加高年龄段权重以惩罚过拟合
        weight = 1.0;
        if ageYears(j) > 70
            weight = 1.3;  % 70岁以上适度加权
        end
        if ageYears(j) > 80
            weight = 1.6;  % 80岁以上适度加权
        end
        RMSI = RMSI + weight * (1 - BM.OutputValues.Cancer_Ov(j)/handles.Variables.Benchmarks.Cancer.Ov_inc(j))^2;
    end
end
RMS_output = RMSI;
...
CoeffsAdv = handles.Variables.Calibration.Step2.CoeffAdv;
CoeffsInc = handles.Variables.Calibration.Step3.CoeffsInc;
...
B(1:3) = output(1:3);
C(1:3) = output(4:6);
handles.Variables.Calibration.Step2.CoeffAdv  = B;
handles.Variables.Calibration.Step3.CoeffsInc = C;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Fallback nlinfit replacement using fminsearch (no toolbox)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function beta = my_nlinfit(X, Y, model, beta0)
    opts = optimset('TolX',1e-6,'TolFun',1e-6,'MaxIter',5000,'MaxFunEvals',10000);
...
handles.Variables.AdvancedProgressionRate = 6*5.1/6.5*0.3e-5*C(1).*exp(-0.01*C(2)*( index_age - C(3)-7 ).^2);
counter = 1;
for x1=1:19
    for x2=1:5
        handles.Variables.AdvancedProgression(counter) = (handles.Variables.AdvancedProgressionRate(x1) * (5-x2) + ...
...
% 辅助函数：带权重的 SSE 计算             %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function sse = weighted_sse(p, u, y, sigmoid_func)
% 计算带权重的平方和误差，首尾数据点权重更高
n = length(u);
y_pred = sigmoid_func(p, u);
weights = ones(1, n);
% 首尾各2个点权重加倍，改善边缘拟合
if n >= 4
    weights(1:2) = 2.5;
...
sse = sum(weights .* (y(:)' - y_pred(:)').^2);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 辅助函数：Richards曲线的加权SSE计算     %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function sse = weighted_sse_richards(p, u, y, richards_func)
% 计算带权重的平方和误差，使用更精细的分段权重
n = length(u);

% 参数约束检查，防止无效参数
if p(5) <= 0 || p(5) > 10  % 不对称参数必须在合理范围
    sse = 1e10;
    return;
end
if p(4) < 0  % 下界不能为负
...
weights = ones(1, n);
if n >= 6
    weights(1) = 4.0;      % 第一个点最高权重
    weights(2) = 3.0;      % 第二个点高权重
    weights(3) = 2.0;      % 第三个点中等权重
    weights(n) = 3.5;      % 最后一个点高权重
    weights(n-1) = 2.5;    % 倒数第二个点中等高权重
    weights(n-2) = 1.5;    % 倒数第三个点略高权重
elseif n >= 4
    weights(1:2) = 3.0;
    weights(n-1:n) = 3.0;
end
