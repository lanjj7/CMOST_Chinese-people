...
if isfield(handles.Variables, 'ScanSettings')
    ...
    handles.Settings.NumberSteps_4   = 5;
...
set(handles.Number_Variables, 'string', num2str(handles.Settings.NumberVariables))
    ...
    if isfield(handles.Settings, 'NumberSteps_4')
        set(handles.Number_Steps_4, 'string', num2str(handles.Settings.NumberSteps_4))
    end
...
elseif isequal(handles.Settings.Scan, '4D')
    Enable(1:4) = 1; Enable(5) = 0;
    set(handles.scan, 'Value', 0)
    set(handles.scan_2D, 'Value', 0)
    set(handles.scan_3D, 'Value', 0)
    if isfield(handles, 'scan_4D')
        set(handles.scan_4D, 'Value', 1)
    end
    set(handles.Number_Variables, 'enable', 'off', 'string', '4')
    set(handles.Number_Steps_2, 'enable', 'on')
    set(handles.Number_Steps_3, 'enable', 'on')
    if isfield(handles, 'Number_Steps_4')
        set(handles.Number_Steps_4, 'enable', 'on')
    end
    
    for f=1:5
        if f<5, tmp = 'on'; else tmp = 'off'; end
        set(handles.(['Variable_' num2str(f)]),    'enable', tmp)
        set(handles.(['Choose_' num2str(f)]),      'enable', tmp)
        set(handles.(['Subposition_' num2str(f)]), 'enable', tmp)
        set(handles.(['min_' num2str(f)]),         'enable', tmp)
...
tmp=get(handles.scan_3D, 'Value');
if isequal(tmp, 1), handles.Settings.Scan = '3D';
else
    handles.Settings.Scan= 'on';
end    
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU> 
function scan_4D_Callback(hObject, eventdata, handles)
tmp=get(handles.scan_4D, 'Value');
if isequal(tmp, 1), handles.Settings.Scan = '4D';
else
    handles.Settings.Scan= 'on';
end    
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU> 
...
function Number_Steps_4_Callback(hObject, eventdata, handles)
tmp=get(handles.Number_Steps_4, 'string');
[num, succ] =str2num(tmp);
if succ
    num = round(num); if num>0,handles.Settings.NumberSteps_4=num; end
end
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
...
tmp=get(handles.linker_repeats, 'string');
handles.Settings.linker_repeats=tmp;
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
...
function min_5_CreateFcn(hObject, eventdata, handles), function filename_CreateFcn(hObject, eventdata, handles), function Number_Steps_3_CreateFcn(hObject, eventdata, handles), function Number_Steps_4_CreateFcn(hObject, eventdata, handles)    
...
% Auto-detect number of configured variables
NumVars = 0;
for f=1:5
    if ~isequal(handles.Settings.Values{f}, 'empty')
        NumVars = f;
    end
end
    ...
    for x1 = 1:handles.Settings.NumberSteps
        ...
        for x2 = 1:EndSteps_2
            ...
            for x3 = 1:EndSteps_3
                ...
                SaveFilenames3D(PathName, handles, FileName, Variables)
            end
        end
    end
elseif NumVars >= 4
        % Use actual Values array lengths for all loops
        NumSteps1 = length(handles.Settings.Values{1});
        NumSteps2 = length(handles.Settings.Values{2});
        NumSteps3 = length(handles.Settings.Values{3});
        NumSteps4 = length(handles.Settings.Values{4});
        
        FolderFlag = true;
        for x1 = 1:NumSteps1
            if FolderFlag
                StringX  = num2str(x1);
                for f=1:(length(num2str(NumSteps1)) - length(StringX))
                    StringX = ['0' StringX];
                end
                tmp = ['Job_series_number_' StringX];
                cd(handles.Settings.SaveDataPath), mkdir(tmp)
            ...
            for x2 = 1:NumSteps2
                for x3 = 1:NumSteps3
                    for x4 = 1:NumSteps4
                        ...
                        Value{2} = handles.Settings.Values{2}{x2};
                        Value{3} = handles.Settings.Values{3}{x3};
                        Value{4} = handles.Settings.Values{4}{x4};
                        for f=1:4
                            if isequal(handles.Settings.SelectionDepth{f}, 1)
                                if handles.Settings.IsVector{f}
                                    Variables.(handles.Settings.Variable{f}{1})(handles.Settings.Subposition{f}) = Value{f};
                        ...
                        StringX  = num2str(x1); StringY  = num2str(x2); StringZ  = num2str(x3); StringW  = num2str(x4);
                        for f=1:(length(num2str(NumSteps1)) - length(StringX))
                            StringX = ['0' StringX];
                        end
                        for f=1:(length(num2str(NumSteps2)) - length(StringY))
                            StringY = ['0' StringY];
                        end
                        for f=1:(length(num2str(NumSteps3)) - length(StringZ))
                            StringZ = ['0' StringZ];
                        end
                        for f=1:(length(num2str(NumSteps4)) - length(StringW))
                            StringW = ['0' StringW];
                        end
                        FileName = [strrep(handles.Settings.Filename, '.mat', ' ') '_'...
                            handles.Settings.Comment{1} '_' strrep(ValueStr{1}{x1}, '.', '_') '_'...
                            handles.Settings.Comment{2} '_' strrep(ValueStr{2}{x2}, '.', '_') '_'...
                            handles.Settings.Comment{3} '_' strrep(ValueStr{3}{x3}, '.', '_') '_'...
                            handles.Settings.Comment{4} '_' strrep(ValueStr{4}{x4}, '.', '_') '.mat'];
                        Variables.Settings_Name = FileName;
                        SaveFilenames3D(PathName, handles, FileName, Variables)
                    end
                end
...
% Save Filenames for 3D/4D modes with custom path
function SaveFilenames3D(PathName, handles, FileName, Variables)
if isequal(handles.Settings.number_repeats, 1)
    ...
    for f=1:handles.Settings.number_repeats
        ...
        FileNameTmp = [FileName LinkerString];
        save(fullfile(PathName, FileNameTmp), 'Variables');
    end
end
...
elseif isstruct(Variables.(SelectionFields{Selection}))
    ...
    for f=1:length(VariablesFields_2)
        if isnumeric(Variables.(String1).(VariablesFields_2{f}))
            ...
            displayParam = VariablesFields_2{f};
            if strcmp(displayParam, 'Sept9_HiSens')
                 displayParam = 'Q+FIT-Sequential Q';
            elseif strcmp(displayParam, 'Sept9_HiSpec')
                 displayParam = 'Q+FIT-Sequential FIT';
            elseif strcmp(displayParam, 'Sept9_HiSens_Sens')
                 displayParam = 'Q+FIT-Sequential Q - Sensitivities';
            elseif strcmp(displayParam, 'Sept9_HiSpec_Sens')
                 displayParam = 'Q+FIT-Sequential FIT - Sensitivities';
            end
            DisplayFields_2{SelectionCounter_2} = displayParam;
        ...
        elseif isstruct(Variables.(String1).(VariablesFields_2{f}))
            ...
            DisplayFields_2{SelectionCounter_2} = VariablesFields_2{f};
            SelectionCounter_2 = SelectionCounter_2 +1;
        end
    end
    [Selection_2, valid] = listdlg('PromptString','Select a Subvariable:',...
        'SelectionMode','single', 'ListString',DisplayFields_2);
    if isequal(valid, 0)
        return
    end
...
elseif isequal(Settings.Scan, '4D')
    if isequal(number, 1)
        NumberSteps = Settings.NumberSteps;
    elseif isequal(number, 2)
        NumberSteps = Settings.NumberSteps_2;
    elseif isequal(number, 3)
        NumberSteps = Settings.NumberSteps_3;
    else
        NumberSteps = Settings.NumberSteps_4;
    end
else
    NumberSteps = Settings.NumberSteps;
end
