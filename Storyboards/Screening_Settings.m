...
% === 修改: 重命名策略 5 的标签 (全面版) ===
% 1. 尝试修改 UIPanel 的标题 (最可能的类型)
allPanels = findall(handles.figure1, 'Type', 'uipanel');
for i = 1:length(allPanels)
    currTitle = get(allPanels(i), 'Title');
    if ischar(currTitle) || isstring(currTitle)
        if contains(currTitle, 'Septin-9 High Sensitivity')
            set(allPanels(i), 'Title', 'Q+FIT-Sequential Q');
        elseif contains(currTitle, 'Septin-9 High Specificity')
            set(allPanels(i), 'Title', 'Q+FIT-Sequential FIT');
        end
    end
end

% 2. 尝试修改 Text 对象的字符串 (文本标签)
allText = findall(handles.figure1, 'Style', 'text');
for i = 1:length(allText)
    currStr = get(allText(i), 'String');
    if iscell(currStr), currStr = currStr{1}; end
    if ischar(currStr) || isstring(currStr)
        if contains(currStr, 'Septin-9 High Sensitivity')
             if contains(currStr, 'Sensitivities') || contains(currStr, 'Ad 1')
                  set(allText(i), 'String', 'Q+FIT-Sequential Q - Sensitivity');
             else
                  set(allText(i), 'String', 'Q+FIT-Sequential Q');
             end
        elseif contains(currStr, 'Septin-9 High Specificity')
             if contains(currStr, 'Sensitivities') || contains(currStr, 'Ad 1')
                  set(allText(i), 'String', 'Q+FIT-Sequential FIT - Sensitivity');
             else
                  set(allText(i), 'String', 'Q+FIT-Sequential FIT');
             end
        end
    end
end
