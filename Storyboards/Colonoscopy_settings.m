...
[num, succ] =str2num(tmp); if succ, if num >=0, handles.Variables.Colonoscopy_RiscPerforation=num; end, end %#ok<ST2NM>
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
function risc_serosa_burn_Callback(hObject, eventdata, handles) 
tmp=get(handles.risc_serosa_burn, 'string');
[num, succ] =str2num(tmp); if succ, if num >=0, handles.Variables.Colonoscopy_RiscSerosaBurn=num; end, end %#ok<ST2NM>
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
function risc_bleeding_Callback(hObject, eventdata, handles) 
tmp=get(handles.risc_bleeding, 'string');
[num, succ] =str2num(tmp); if succ, if num >=0, handles.Variables.Colonoscopy_RiscBleeding=num; end, end %#ok<ST2NM>
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
function risc_severe_bleeding_Callback(hObject, eventdata, handles) 
tmp=get(handles.risc_severe_bleeding, 'string');
[num, succ] =str2num(tmp); if succ, if num >=0, handles.Variables.Colonoscopy_RiscBleedingTransfusion=num; end, end %#ok<ST2NM>
handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>
