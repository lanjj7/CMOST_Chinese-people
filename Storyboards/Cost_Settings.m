...
% === 修改: 重命名 Sept-9 High Sens 的标签 ===
allText = findall(handles.figure1, 'Style', 'text');
for i = 1:length(allText)
    % 重命名左侧的成本标签
    if strcmp(get(allText(i), 'String'), 'Septin-9 High Sensitivity') || ...
       strcmp(get(allText(i), 'String'), 'Septin 9 High Sens.') || ... % GUI 中可能的简写
       contains(get(allText(i), 'String'), 'Septin 9 High Sens')
        set(allText(i), 'String', 'Q+FIT-Sequential');
    end
end
% ===========================================
